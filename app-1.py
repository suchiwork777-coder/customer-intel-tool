...
# LLM SYNTHESIS
# ---------------------------
def build_prompt(company_name, signals, account_context=None):
    signal_text = ""
    ...
    context_text = ""
    if account_context and any(account_context.values()):
        context_text = "\n## Internal account context (from the CSM, not public)\n"
        if account_context.get("renewal_date"):
            context_text += f"- Renewal date: {account_context['renewal_date']}\n"
        if account_context.get("arr"):
            context_text += f"- Account value (ARR): {account_context['arr']}\n"
        if account_context.get("stakeholder"):
            context_text += f"- Key stakeholder: {account_context['stakeholder']}\n"
        if account_context.get("notes"):
            context_text += f"- Other notes: {account_context['notes']}\n"
...
Using the signals above (and the internal account context if provided), produce a concise
daily brief with exactly these sections:
...
- If internal account context is provided (renewal date, ARR, stakeholder), explicitly connect
  at least one public signal to that context (e.g. a signal that raises renewal risk or an
  expansion opportunity given the account value).

### 3. Suggested talk track
- Write ONE short sentence (max 30 words) the CSM could literally say to open a conversation,
  referencing the most relevant signal. If a stakeholder name is provided, address it to them.

If there are no strong signals, say so honestly instead of inventing information.
Do not fabricate facts not present in the signals or context above."""
    return prompt
...
with st.expander("➕ Add account context (optional, makes the brief more personalized)"):
    ctx_col1, ctx_col2 = st.columns(2)
    with ctx_col1:
        renewal_date = st.text_input("Renewal date", placeholder="e.g. Nov 2026")
        arr = st.text_input("Account value / ARR", placeholder="e.g. $250K")
    with ctx_col2:
        stakeholder = st.text_input("Key stakeholder", placeholder="e.g. Priya Shah, VP Ops")
        notes = st.text_area("Other notes", placeholder="e.g. flagged slow support response last month")
    ...
    if not company_name.strip():
        ...
        if total_signals == 0:
            ...
            with st.spinner("Synthesizing brief with AI..."):
                ...
                prompt = build_prompt(company_name, signals, account_context)
                try:
